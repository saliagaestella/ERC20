...
contract XXXTOKEN is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    XXXTOKENDividendTracker public dividendTracker;
    ...
    constructor() BEP20("XXXTOKEN", "XXX") 
    {
        dividendTracker = new XXXTOKENDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "XXXTOKEN: The dividend tracker already has that address");
        XXXTOKENDividendTracker newDividendTracker = XXXTOKENDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "XXXTOKEN: The new dividend tracker must be owned by the XXXTOKEN token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "XXXTOKEN: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "XXXTOKEN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XXXTOKEN: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "XXXTOKEN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "XXXTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "XXXTOKEN: Cannot update gasForProcessing to same value");
...
contract XXXTOKENDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("XXXTOKEN_Dividend_Tracker", "XXXTOKEN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "XXXTOKEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "XXXTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XXXTOKEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "XXXTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "XXXTOKEN_Dividend_Tracker: Cannot update claimWait to same value");
